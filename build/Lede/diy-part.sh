...
# 后台IP设置
export Ipv4_ipaddr="172.17.1.254"            # 修改openwrt后台地址(填0为关闭)
...
version.buildinfo
profiles.json
openwrt-xiaomi_redmi-router-ax6000-$(TZ=UTC-8 date "+%Y.%m.%d")-kernel.bin
openwrt-xiaomi_redmi-router-ax6000-$(TZ=UTC-8 date "+%Y.%m.%d").manifest
openwrt-xiaomi_redmi-router-ax6000-$(TZ=UTC-8 date "+%Y.%m.%d")-squashfs-rootfs.img.gz
EOF
